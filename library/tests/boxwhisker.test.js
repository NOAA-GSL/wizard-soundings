...
describe('SkewT Box Whisker - Percentile Computation', () => {
    ...
    const createMember = (tempOffset, dwptOffset) => [
        { press: 1000, temp: 20 + tempOffset, dwpt: 10 + dwptOffset },
        { press: 850, temp: 10 + tempOffset, dwpt: 2 + dwptOffset },
        { press: 500, temp: -15 + tempOffset, dwpt: -25 + dwptOffset },
    ];
    ...
    test('returns null for empty input', () => {
        expect(computePercentileProfiles([], [5, 25, 75, 95])).toBeNull();
        expect(computePercentileProfiles(null, [5, 25, 75, 95])).toBeNull();
    });

    test('computes all requested percentiles plus median', () => {
        const result = computePercentileProfiles(memberProfiles, [5, 25, 75, 95]);
    ...
    test('median is the correct value for uniform distribution', () => {
        const result = computePercentileProfiles(memberProfiles, [25, 75]);
    ...
    test('handles members with missing data gracefully', () => {
        const membersWithGaps = [
            [{ press: 1000, temp: 20, dwpt: 10 }, { press: 850, temp: null, dwpt: 5 }],
            [{ press: 1000, temp: 22, dwpt: 12 }, { press: 850, temp: 12, dwpt: 4 }],
            [{ press: 1000, temp: 18, dwpt: 8 }, { press: 850, temp: 8, dwpt: 2 }],
        ];
        const result = computePercentileProfiles(membersWithGaps, [25, 75]);
        expect(result).not.toBeNull();
